function recorrerArreglo(detalles) {  
        ...
        if(documento_orden == (data.Cod_TipoComprobante+':'+data.Serie)){
            ...
            if(!dataDocumento.hasOwnProperty(documento_orden)){
                ...
                dataDocumento[documento_orden]={
                    "html":'<tr class="row">'+
                                    '<td colspan="6" style="font-weight: bold;color:#4285f4;font-size: 15px"> Documento: '+data.Cod_TipoComprobante+' '+data.Nom_TipoComprobante+' Serie: '+data.Serie+'</td>'+
                            '</tr>', 
                    "formas_pago":{}
                }
            ...
            dataDocumento[keyD]['formas_pago'][keyFP].forEach(function(dataHtml,index){
                ...
                if(index==(dataDocumento[keyD]['formas_pago'][keyFP].length-1)){
                      html_forma_pago = html_forma_pago +   '<tr class="row">'+ 
                                                                ...
                                                                '<td></td>'+
                                                                '<td></td>'+
                                                                '<td class="tablaDetalleAlert" style="font-weight: bold;text-align:left">SUB-TOTAL ('+dataHtml['Nombre_FormaPago']+')</td>'+
    ...
    html_general = html_general+'<tr class="row">'+ 
                ...
                '<td></td>'+
                '<td></td>'+
                '<td style="font-weight: bold;color:red;text-align:left;font-size: 15px">TOTAL GENERAL : </td>'+
